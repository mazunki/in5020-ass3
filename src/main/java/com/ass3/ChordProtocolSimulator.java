...
public class ChordProtocolSimulator {
	 ...
	 *
	 * First keys are generated randomly. Each key is assigned a key name and
	 * its store in the keyIndexes. Then keyIndexes are added to the node.
	 *
	 * For each key index:
	 *   1) find the node that should be responsible for the key (the index of
	 *      the node should be greater than the index of key and the key index
	 *      should be close to node index in the consistent hash ring)
	 *   2) add the key index to the node (by calling the peer.add_data() from
	 *      the network)
	 */
	...
	/**
	 * This method finds peer that should be responsible for a given key. It
	 * retrieves the set of nodes from the network and for each node calculates
	 * the index of the node using consistent hashing. Then it compares the
	 * index of the key with index of all the nodes. It choose a node that has
	 * an index that would be next to the key index in the consistent hash
	 * ring.
	 *
	...
	public String findPeer(int key_index) {
		...
		// check if the key index is larger than the biggest node index. if it
		// is larger then the key should be placed before the lowest node index
		// (at the start of the ring)
		if (key_index > sortedIndexes.get(sortedIndexes.size() - 1)) {
			peerIndex = sortedIndexes.get(0);
		} else {
			// if the key index is not larger than the biggest node index, then
			// choose the node that has index bigger than the key index. loop
			// through the nodes in the ascending order of the node indexes
		...
		// if no node is found then return null. Note: If everything works well
	...
	/**
	 * This method returns the peer name based on the given peer index. This
	 * method uses a hack that the network setup uses the names Peer 1, Peer
	 * 2, etc. It would be nice if this was automatic. Like fetching all the
	 * nodes from the network and compare the names with the given node (based
	 * on index). Maybe I can improve it later.
	 *
	 ...
	 * prints different information contained in the node such as neighbors,
	 * routing table, data.
	 */
	 ...
	 * circular linked list algorithm to travel the network. It starts with
	 * head and goes through successors. I am assuming that each peer stores
	 * only one neighbor but i think the chord protocol doesn't make this
	 * restriction. but still even if the node has multiple neighbors this
	...
	/**
	 * This method tests the functioning of the lookup. This is a simple
	 * evaluation. For each key index it calls the lookup from the chord
	 * protocol and returns the node index. It then compares the node index
	...
	/**
	 * This method compares whether the node actually stores the given key
	 * index or not. It retrieves the data items stored at the particular node
	 * and compares whether the key index is stored or not
	 * 
